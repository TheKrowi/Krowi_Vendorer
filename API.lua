...
do --[[ KrowiV_RegisterAutoJunkOptions ]]
    local function InjectOptionsDefaults(expansion, instanceId, junkByDefault)
		...
		if not KrowiV_InjectOptions:DefaultsExists("AutoJunk.Instances") then
			KrowiV_InjectOptions:AddDefaults("AutoJunk", "Instances", { });
		end
		KrowiV_InjectOptions:AddDefaults("AutoJunk.Instances", instanceId, junkByDefault);
	end
	...
	local function InjectOptionsTable(expansion, expansionDisplayName, instanceType, instanceTypeDisplayName, instanceId, instanceDisplayName)
        if not KrowiV_InjectOptions:TableExists("AutoJunk.args." .. expansion) then
			KrowiV_InjectOptions:AddTable("AutoJunk.args", expansion, {
				order = OrderPP(), type = "group",
				name = expansionDisplayName,
				args = {}
			});
		end
		if not KrowiV_InjectOptions:TableExists("AutoJunk.args." .. expansion .. ".args." .. instanceType) then
			KrowiV_InjectOptions:AddTable("AutoJunk.args." .. expansion .. ".args", instanceType, {
				order = OrderPP(), type = "group", inline = true,
				name = instanceTypeDisplayName,
				args = {}
			});
		end
		KrowiV_InjectOptions:AddTable("AutoJunk.args." .. expansion .. ".args." .. instanceType .. ".args", tostring(instanceId), {
    ...
    function KrowiAF_RegisterDeSelectAllEventOptions(expansion, instanceType, instanceIds)
		if KrowiV_InjectOptions:TableExists("AutoJunk.args." .. expansion .. ".args." .. instanceType .. ".args.SelectAll") then
			return;
		end

		KrowiV_InjectOptions:AddTable("AutoJunk.args." .. expansion .. ".args." .. instanceType .. ".args", "Blank1", {
			order = OrderPP(), type = "description", width = "full", name = ""
		});
		KrowiV_InjectOptions:AddTable("AutoJunk.args." .. expansion .. ".args." .. instanceType .. ".args", "Blank2", {
			order = OrderPP(), type = "description", width = AdjustedWidth(0.95), name = ""
		});
		KrowiV_InjectOptions:AddTable("AutoJunk.args." .. expansion .. ".args." .. instanceType .. ".args", "SelectAll", {
		...
		KrowiV_InjectOptions:AddTable("AutoJunk.args." .. expansion .. ".args." .. instanceType .. ".args", "DeselectAll", {
			order = OrderPP(), type = "execute", width = AdjustedWidth(0.95),
